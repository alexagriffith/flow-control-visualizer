...
export function TrafficChart({ traffic, source }: Pick<ProgressData, 'traffic' | 'source'>) {
  ...
  return <section className="progress-panel traffic-panel">
    ...
    {traffic.unavailable ? <p className="progress-note">{traffic.unavailable}</p> : <>
      <div className="progress-chart-label"><span>Recorded request starts/s · {width}s bins</span><span>{source === 'harness' ? 'Latest saved attempt · all streams' : 'Native request records'}</span></div>
    ...
    {traffic.partial ? <p className="progress-alert">Partial export: missing, incomplete or oversized records are not shown.</p> : null}
  </section>
}
